...
import { type Dealer, DealerSchema } from "@/lib/types/dealer";

export async function getDealerByBranch(branchId: string): Promise<Dealer[]> {
  const supabase = createClient();
  const { data: dealers, error } = await supabase
    .from("branch_dealer_mapping")
    .select("dealer(id, name, is_enable, tipe_dealer, address, pic)")
  ...
  return z.array(DealerSchema).parse(dealers.map((item) => item.dealer));
}
